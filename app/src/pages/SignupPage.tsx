...
import {
  UserPlus, Lock, User, Loader2, Eye, EyeOff, ChevronLeft, AtSign,
} from 'lucide-react';
...
function FeaturePills() {
  const pills = ['50 free prompts/day', 'Cloud sync', 'No credit card'];
  return (
    <div className="flex flex-wrap gap-2 mt-6">
      {pills.map((p) => (
        <span
          key={p}
          className="px-2.5 py-1 rounded-full text-[10px] font-medium border border-white/[0.08] text-white/40"
        >
          ✓ {p}
        </span>
      ))}
  ...
  return (
    <main className="relative md:h-screen md:overflow-hidden lg:grid lg:grid-cols-2" style={{ backgroundColor: 'var(--m-bg-base)' }}>
      ...
      <div className="relative hidden h-full flex-col border-r border-white/[0.06] p-10 lg:flex overflow-hidden bg-[#07070d]">
        <div className="absolute inset-0 z-10 bg-gradient-to-t from-[#07070d]/80 via-transparent to-transparent" />
        ...
        <div className="z-20 mt-auto space-y-8">
          <div className="grid grid-cols-2 gap-4">
            {[
              { value: '50', label: 'Free prompts daily' },
              { value: '30+', label: 'AI capabilities' },
              { value: '100+', label: 'Languages' },
              { value: '5k+', label: 'Active users' },
            ].map(({ value, label }) => (
              <div key={label} className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm">
          ...
          <blockquote className="space-y-2">
            <p className="text-[16px] text-white/70 leading-relaxed font-light">
              &ldquo;The best AI workspace I&apos;ve used. Fast, accurate, and
              the free tier is genuinely useful.&rdquo;
            </p>
            <footer className="font-mono text-sm font-semibold text-[var(--m-accent-blue)]">
              ~ Marcus Johnson, PM @ Linear
            </footer>
          </blockquote>
        </div>
      </div>
      ...
      <div className="relative flex h-screen overflow-y-auto flex-col p-6 sm:p-10">
        ...
        <div className="pointer-events-none absolute inset-0 -z-10">
          <div className="absolute top-0 right-0 h-80 w-80 rounded-full bg-[#a78bfa]/5 blur-3xl -translate-y-1/3 translate-x-1/3" />
          <div className="absolute bottom-0 left-0 h-60 w-60 rounded-full bg-[var(--m-accent-blue)]/5 blur-3xl translate-y-1/3 -translate-x-1/3" />
        </div>
        ...
        >
          ...
          <div className="space-y-1 mt-6 text-center">
            ...
            <p className="text-[14px] text-[var(--m-text-muted)]">
              Get 50 free prompts per day with cloud sync.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="relative">
              ...
              />
              <User size={15} strokeWidth={1.5} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--m-text-muted)]" />
            </div>
            ...
            >
              {isLoading ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} strokeWidth={1.5} />}
          ...
          <p className="text-center text-[13px] text-[var(--m-text-muted)]">
            ...
            <Link to="/login" className="text-[var(--m-accent-blue)] font-medium hover:underline">
              Sign in
            </Link>
          </p>
        </motion.div>
