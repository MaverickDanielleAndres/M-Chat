    ...
    <div className="flex items-center gap-0.5">
      {Array.from({ length: count }).map((_, i) => (
        <Star
          key={i}
          size={12}
          className="text-muted-foreground fill-[var(--m-text-secondary)]"
        />
      ))}
    </div>
  );
  ...
  return (
    <section className="py-16 md:py-24 relative overflow-hidden" style={{ backgroundColor: 'var(--m-bg-base)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-indigo-500 mb-3">
              Social Proof
            </p>
            <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground tracking-tight leading-[1.1]">
              Loved by creators
              <br />
              everywhere.
            </h2>
          ...
          <div className="flex gap-10 md:gap-14 border-t md:border-t-0 md:border-l border-white/10 pt-5 md:pt-0 md:pl-14">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground mb-1">5k+</p>
              <p className="text-[9px] text-muted-foreground uppercase tracking-wider">happy users</p>
            </div>
          </div>
        </motion.div>
          ...
          <div className="flex animate-marquee gap-4 whitespace-nowrap min-w-max hover:[animation-play-state:paused]">
            {[...testimonials, ...testimonials].map((testimonial, i) => (
              <div
                ...
                className="w-[320px] shrink-0 group relative rounded-2xl border border-border bg-card/50 hover:border-border hover:-translate-y-1 transition-all duration-300 hover:shadow-xl hover:shadow-black/30 whitespace-normal flex flex-col"
              >
                  <div className="p-6">
                    <Quote className="text-foreground/[0.04] fill-white/[0.04] w-8 h-8 mb-4 rotate-180" />
                    <StarRating count={5} />
                    <p className="text-[13px] text-muted-foreground leading-relaxed mt-4 line-clamp-4">
                      {testimonial.content}
                    </p>
                  </div>
                  ...
                  <div className="p-6 pt-0 mt-auto flex items-center gap-3">
                    <div 
                      className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-foreground/90 bg-white/10"
                    >
